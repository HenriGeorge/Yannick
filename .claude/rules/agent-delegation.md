...
- **auditor** — the P5 REVIEW panel over a diff, dispatched **CONCURRENTLY — all four in ONE
  message**: **required** `code-reviewer` (owns the verdict) + `silent-failure-hunter`
  (error-handling/fallbacks; may escalate); **advisory** `code-simplifier` (behavior-preserving
  cleanups) + `comment-analyzer` (comment rot) — findings in the same single comment, never their
  own marker; on docs-only diffs they report "no applicable findings". Reviews `git diff`, tries
  to break it. If the panel changes code, re-run VERIFY before merge.
...
- **browser-tester** — run tests against the *running* app on the worktree's port: `playwright-tester`
  ...
  (a contended run prints the holder and exits 75). Missing `.env.test` → fail loud; never test unauthenticated.
